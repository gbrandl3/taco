...
void usage(const char *cmd)
{
        fprintf(stderr, "usage : %s [options] <device name>\n", cmd);
      	fprintf(stderr, " control one or more devices\n");
...
long devcmd(char *devname)
{
   ...
   if(status<0)
   {
      ...
      status= dev_putget(ds,DevRemote,NULL,D_VOID_TYPE,&vgc_status,D_VGC_STATUS,&error);
      if(status<0) printf("** %s: %s **\n",devname,dev_error_str(error)+25);
      else printf(" %s switched in remote \n",devname);
   dev_free(ds,&error);

 ...
 */
   ...
   printf("device name	state	set point	read point\n");
   for(i=0;i<dev_nb;i++)
   {
      devcmd(device_tab[i]);
   }
...
long devcmd(char *devname)
{
   ...
   status= dev_putget(ds,DevUpdate,NULL,D_VOID_TYPE,&state,D_STATE_FLOAT_READPOINT,&error);
   if(status==0)
   {
      printf("%s:	%s",devname,DEVSTATES[state.state]);

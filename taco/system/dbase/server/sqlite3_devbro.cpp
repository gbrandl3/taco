 ...
 */
db_res *SQLite3Server::devdomainlist_1_svc(void)
{
...
//
	...
	logStream->infoStream() << "SQLite3Server::devdomainlist_1_svc(): query " << query << log4cpp::eol;
 ...
 */
db_res *SQLite3Server::devfamilylist_1_svc(DevString * domain)
{
...
//
	...
	logStream->infoStream() << "SQLite3Server::devfamilylist_1_svc(): query " << query << log4cpp::eol;
	if (sqlite3_get_table(db, query.c_str(), &result, &nrow, &ncol, &zErrMsg) != SQLITE_OK)
	{
		logStream->errorStream() << sqlite3_errmsg(db) << log4cpp::eol;
		browse_back.db_err = DbErr_DatabaseAccess;
 ...
 */
db_res *SQLite3Server::devmemberlist_1_svc(db_res *recev)
{
...
//
	...
	logStream->debugStream() << "In devmemberlist_1_svc function for domain " << user_domain 
				<< " and family " << user_family << log4cpp::eol;
//
// Initialize structure sent back to client
//
	browse_back.db_err = 0;

...
static void compute_mask(int pos,int nb_bit,unsigned char *pmask);


/**
 * To allocate place in the shared memory area 
 ...
 * is 0, the area is free for use. To allocate place means to find enough 
 * consecutive bits at zero, to change them to one and to return to the caller 
 ...
 * 
 * @param base Base address of the data buffer
 * @param tab Address of the allocation table
 * @param size The buffer size to allocate (must be a multiple of BLOS_SIZE)
 * @param alloc_size The allocation area size (in bytes)
 * @param semid The semaphore identifier
 * @param perr The pointer to the error
 *
 * @return This function returns NULL if one error occurs and the error code will
...
/**
 * To compute the mask needed (for an OR operation) to set the necessary bits 
 * in the first byte of the area to be allocated
 * 
 * @param pos The position of the first bit to set (between 1 and 8)
 * @param nb_bit The number of bits to set (between 1 and 8)
 *
 * @param pask The pointer to the calculated mask
 *
 */

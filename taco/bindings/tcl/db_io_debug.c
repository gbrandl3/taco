...
/* --------------------------------------------------------------- */
/* =============================================================== */
int db_io_debug (clientdata,interp,argc,argv)
/*SUPPRESS761*/
    ...
    char *argv[];
{
 ...
 if (ReturnedCode==TCL_ERROR)
    {
     Tcl_ResetResult(interp);
     aux = (char *)malloc(80);
     sprintf (aux,"ERROR in db_io_debug : bad argin %s",argv[1]);
 ...
 if (ReturnedCode==False)
    {
     Tcl_ResetResult(interp);
     sprintf (aux,"ERROR in db_io_debug : bad value in argin %d",Value);
     Tcl_AppendResult (interp,aux,NULL);
     return TCL_ERROR;
    }

...
/* --------------------------------------------------------------- */
int dc_io_get_hist (clientdata, interp, argc, argv)
/*SUPPRESS761*/
...
/* --------------------------------------------------------------- */
int dc_io_get (clientdata, interp, argc, argv)
              ClientData clientdata;
              Tcl_Interp *interp;
              int argc;
              char *argv[]; 
              
{
 ...
 char *State,*Error,*DeviceName,*CommandName,*ArginName; /* Matched with the arguments of the command line */
...
/* int TCL_TEST=False;  */
 ...
 /* Setting debug mode               */
 /* -------------------------------- */
 if (GetDCDebugMode() != DEBUG_LEVEL_0) TCL_TEST=True;
 ...
 /* ------------------------------------------------- */
 if (FirstCall==True) {
  if (TCL_TEST) printf("dc_io_get. First call...\n"); 
  FirstCall=False;
 ...
 /* Parse argument line  */
 if (TCL_TEST) printf("dc_io_get. Parse argument line...\n");
 ...
 DeviceName  = argv[3];
 CommandName = argv[4];
 if (DC_MODE==DC_HISTORY_MODE_ON) ArginName=argv[5];
 
 if (TCL_TEST) printf("dc_io_get. Argument line parsed\n");
 ...
 StringTemp = (char*) calloc (500,sizeof(char));
 OutputArg = NULL;

 if (TCL_TEST) printf("dc_io_get. variable init...Done\n");
 
 
 /* If this device is not already imported, do it*/
 if (TCL_TEST) printf("dc_io_get. Check if the device %s is already imported\n",DeviceName);
 ...
 do
   {
    ...
    if (test1==0) break; /* This device is already imported */
 ...
 {
   if (TCL_TEST) printf("dc_io_get. dc_import %s\n",DeviceName);
   ...
   dc_imp.dc_dev_error = &TmpLong;
     ...
     if (DeviceStatus != DS_OK) /* Error in the import */
     {
       ...
       sprintf (StringTemp,"%s [%d]",StringAux,*(dc_imp.dc_dev_error));
       Tcl_SetVar (interp,Error,StringTemp,0);          
     	 if (TCL_TEST) printf("ERROR during the import :%s\n",StringTemp);
 ...
 {
   if (TCL_TEST) printf("The device %s is already imported\n",DeviceName);
 }
     
 /* Test if the command is available */
 if (TCL_TEST) printf("dc_io_get. Check if the command %s is available\n",CommandName); 
 ...
 {
   Tcl_ResetResult(interp);
   sprintf(StringTemp,"The %s command is not available for dc_io_get\n",CommandName);
   Tcl_AppendResult(interp,"ERROR in dc_io_get...\n",StringTemp,NULL);
   free (StringTemp);
   return TCL_ERROR;
 }
 
 if (TCL_TEST) printf("dc_io_get. Ready to call dc_devget\n");
 ...
 /* ------------------------------------------------------ */
 if (TCL_TEST) printf("Preparing output variable...");
 
 OutputArg = NULL;
 switch (ArgoutRequired)
        {
        case D_VOID_TYPE         	:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VOID_TYPE\n");
                 break;
        case D_BOOLEAN_TYPE      	:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_BOOLEAN_TYPE\n");
                 F_D_BOOLEAN_TYPE(interp,"",OUTPUT);
                 break;
        case D_USHORT_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_USHORT_TYPE\n");
        				 F_D_USHORT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_SHORT_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_SHORT_TYPE\n");
        				 F_D_SHORT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_ULONG_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_ULONG_TYPE\n");
        				 F_D_ULONG_TYPE(interp,"",OUTPUT);
        				 break;
        case D_LONG_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_LONG_TYPE\n");
        				 F_D_LONG_TYPE(interp,"",OUTPUT);
        				 break;
        case D_FLOAT_TYPE		: 
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_FLOAT_TYPE\n");
        				 F_D_FLOAT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_DOUBLE_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_DOUBLE_TYPE\n");
        				 F_D_DOUBLE_TYPE(interp,"",OUTPUT);
        				 break;
        case D_STRING_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_STRING_TYPE\n");
        				 F_D_STRING_TYPE(interp,"",OUTPUT);
        				 break;
        case D_INT_FLOAT_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_INT_FLOAT_TYPE\n");
        				 F_D_INT_FLOAT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_FLOAT_READPOINT		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_FLOAT_READPOINT\n");
        				 F_D_FLOAT_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_STATE_FLOAT_READPOINT	:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_STATE_FLOAT_READPOINT\n");
        				 F_D_STATE_FLOAT_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_LONG_READPOINT		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_LONG_READPOINT\n");
        				 F_D_LONG_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_DOUBLE_READPOINT		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_DOUBLE_READPOINT\n");
        				 F_D_DOUBLE_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_VAR_CHARARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_CHARARR\n");
        				 F_D_VAR_CHARARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_STRINGARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_STRINGARR\n");
        				 F_D_VAR_STRINGARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_USHORTARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_USHORTARR\n");
        				 F_D_VAR_USHORTARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_SHORTARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_SHORTARR	\n");
        				 F_D_VAR_SHORTARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_ULONGARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_ULONGARR\n");
        				 F_D_VAR_ULONGARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_LONGARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_LONGARR\n");
        				 F_D_VAR_LONGARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_FLOATARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_FLOATARR\n");
        				 F_D_VAR_FLOATARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_DOUBLEARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_DOUBLEARR\n");
        				 F_D_VAR_DOUBLEARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_FRPARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_FRPARR\n");
        				 F_D_VAR_FRPARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_SFRPARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_SFRPARR\n");
        				 F_D_VAR_SFRPARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_LRPARR		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_VAR_LRPARR\n");
        				 F_D_VAR_LRPARR(interp,"",OUTPUT);
        				 break;
        case D_OPAQUE_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_OPAQUE_TYPE\n");
        				 F_D_OPAQUE_TYPE(interp,"",OUTPUT);
        				 break;
        case D_MULMOVE_TYPE		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_MULMOVE_TYPE	\n");
        				 F_D_MULMOVE_TYPE(interp,"",OUTPUT);
        				 break;
        case D_MOTOR_LONG		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_MOTOR_LONG\n");
        				 F_D_MOTOR_LONG(interp,"",OUTPUT);
        				 break;
        case D_MOTOR_FLOAT		:
                 if(TCL_TEST) printf("dc_io_get: ArgoutRequired= D_MOTOR_FLOAT\n");
 ...
 if (DC_MODE==DC_HISTORY_MODE_OFF)
 {
     ...
     if (DeviceStatus != DS_OK) /* Error in the dc_devget */
       {
  			 ...
  			 Tcl_SetVar (interp,Error,StringTemp,0);
  			 free(StringAux);
  			 free (StringTemp); free (OutputArg);
  			 return TCL_OK;
        }
		 else {
		    if (TCL_TEST) printf("dc_ioget. dc_devget succeeded\n"); 
		 }
  ...
  else /* history_mode */
  {
    ...
    if (DeviceStatus != DS_OK) /* Error in the dc_devget */
    {
  		...
  		sprintf (StringTemp,"%s [%d]",StringAux,DCHist.cmd_error);
  		Tcl_SetVar (interp,Error,StringTemp,0);
  		free(StringAux);
  		free (StringTemp); free (OutputArg);         
     	if (TCL_TEST) printf("exiting\n");     					  	
      return TCL_OK;
    }
...
/**************************************************************************/                    
        ...
        case D_STATE_FLOAT_READPOINT	:
                 ...
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%f",(((DevStateFloatReadPoint*)OutputArg)->set));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%f",(((DevStateFloatReadPoint*)OutputArg)->read));
                 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_LONG_READPOINT		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
 ...
 if (DC_MODE==DC_HISTORY_MODE_ON)
    {
     ...
     strcpy(TclAux,ctime(&(DCHist.time))); /* Converting time to string */
     R = strlen (TclAux);

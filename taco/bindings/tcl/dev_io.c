static char RcsId[] = 
"@(#)$Header: /home/jkrueger1/sources/taco/backup/taco/bindings/tcl/dev_io.c,v 1.1 2003-03-18 16:28:29 jkrueger1 Exp $";
/*
 * File : 	dev_io.c
 *
...
/*SUPPRESS761*/
{
 ...
 char *State,*Error,*DeviceName,*CommandName; /* matched with arguments of the command line */
 int i,j;
 int test1,test2;
 long DeviceStatus; /* To obtain the state of a device */
 devserver ds;
 long DeviceError; /* To have the error of a device */
 char *StringTemp=NULL,*StringAux=NULL;
 ...
 DevType ArginRequired,ArgoutRequired; /* For knowing the types needed by the device's command */
 ...
 char format[32] ;   /* to build the format when call sprintf to convert */
  ...
  /* printf("tcl_precision= %d\n",precision); fflush stdout; */
 
 /* -------------------------------- */
 /* Setting debug mode               */
 /* -------------------------------- */
 if (GetDebugMode() != DEBUG_LEVEL_0) {
   TCL_TEST=True;
 }
 else {TCL_TEST=False;} 
  

if (TCL_TEST) {printf("++++++++++++ Entering dev_io ++++++++++++++++\n");}
...
if (FirstCall==True)
 {
  if (TCL_TEST) {printf("First call of dev_io\n");} 
  FirstCall=False;
 ...
 /* ------------------------------------------------- */
 if (TCL_TEST) {printf("Verifying the number of the input arguments\n");} 
 ...
 DeviceName  = argv[3];
 CommandName = argv[4];
 if (TCL_TEST) {printf("Devicename=%s CommandName=%s\n",DeviceName , CommandName); }
 ...
 if (TCL_TEST) {printf("Initialization of dynamical temporary variables\n");}
 StringTemp = (char*) calloc (250,sizeof(char)); /* Necessary for the sprintf function */
 SequencePointer = NULL;
 TclAux = NULL;
InputArg = NULL; 
 OutputArg = NULL;
 StringType = NULL;
if (TCL_TEST) {printf("StringTemp=%X SequencePointer=%X TclAux=%X\n InputArg=%X OutputArg=%X StringType =%X\n", \
                       StringTemp,SequencePointer,TclAux ,InputArg,OutputArg,StringType);}
 
 /* ----------------------------------*/
 /* Import the device if mot done yet */
 /* ----------------------------------*/
 if (TCL_TEST) {printf("check if the device: %s is already imported\n",DeviceName);}
 ...
 if (test1 != 0)  /* We are dealing with a new Device */
    {
     NEW_DEVICE=True;
     if (TCL_TEST) {printf("%s is not registered as imported. send dev_import\n",DeviceName);}
     DeviceError = 0;
     DeviceStatus = dev_import (DeviceName,readwrite,&ds,&DeviceError);     
     if (TCL_TEST) {printf("error_import= %d\n",DeviceError);}
     if (DeviceStatus != DS_OK) /* dev_import failed */
     {
        if (TCL_TEST) { printf("dev_import failed\n"); }
         sprintf (StringTemp,"%d",DeviceStatus);
      ...
      */
     if (TCL_TEST) { printf("Call dev_cmd_query to check if the device is successfully imported\n"); }
     DeviceStatus = dev_cmd_query (ds,&Info,&DeviceError);
     if (DeviceStatus != DS_OK) /* dev_cmd_query failed */
        {
        if (TCL_TEST) { printf("dev_cmd_query failed\n"); }
      ...
      if (TCL_TEST) { printf("dev_cmd_query OK for %s\n",DeviceName); }
  ...
  {
    if (TCL_TEST) {printf("%s was freed, import it again.\n",DeviceName); }
    DeviceError = 0;
  ...
  if (TCL_TEST) {printf("%s is already imported..\n",DeviceName); }
...
/* ---------------------------------- */
if (TCL_TEST) {printf("Testing if the command is '?'....\n"); }
...
for (i=0;i<Info.length;i++)
{
  ...
  if (InType==NULL)  InType = "unknown_type";
  if (OutType==NULL) OutType = "unknown_type";
 
  /* free the in_name and out_name variables */
  if (InfoCmd->in_name  != NULL) free (InfoCmd->in_name);
  if (InfoCmd->out_name != NULL) free (InfoCmd->out_name);
  ...
  if (strcmp(CommandName,InfoCmd->cmd_name)==0)
  {
    ...
    if (TCL_TEST) {printf("%s is a valid command for %s\n",CommandName,DeviceName);}
    if (TCL_TEST) {printf("ArginRequired=%d ArgoutRequired=%d\n",ArginRequired,ArgoutRequired);}
...
if (TCL_TEST) {printf("Ckeck if command %s is valid\n",CommandName);}
if (test2==False) 
{
  /* Not a valid command */
  sprintf (StringTemp,"%d",DS_NOTOK);
  Tcl_SetVar (interp,State,StringTemp,0);
  StringAux = dev_error_str(34);
  sprintf (StringTemp,"%s [%d]",StringAux,34);
  Tcl_SetVar (interp,Error,StringTemp,0);
  free(StringAux); free (StringTemp);
  return TCL_OK;
}   
...
/* ------------------------------------------ */
if (TCL_TEST) {printf("Check if an input argument is required..\n");}
     ...
     (ArginRequired==False && argc==NUMBER_ARGUMENTS_MAX+1) )
{
  ...
  */
  ...
  sprintf (StringTemp,"%s [%d]",StringAux,DevErr_IncompatibleCmdArgumentTypes);
  Tcl_SetVar (interp,Error,StringTemp,0);
  free(StringAux); free (StringTemp);
  return TCL_OK;
}
...
/* ------------------------------------------------------ */
if (TCL_TEST) {printf("Setting the ouput argument in a good format..\n");}
OutputArg = NULL;
switch (ArgoutRequired)
       {
        case D_VOID_TYPE         	:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VOID_TYPE\n");
                 break;
        case D_BOOLEAN_TYPE      	:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_BOOLEAN_TYPE\n");
                 F_D_BOOLEAN_TYPE(interp,"",OUTPUT);
                 break;
        case D_USHORT_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_USHORT_TYPE\n");
        				 F_D_USHORT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_SHORT_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_SHORT_TYPE\n");
        				 F_D_SHORT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_ULONG_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_ULONG_TYPE\n");
        				 F_D_ULONG_TYPE(interp,"",OUTPUT);
        				 break;
        case D_LONG_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_LONG_TYPE\n");
        				 F_D_LONG_TYPE(interp,"",OUTPUT);
        				 break;
        case D_FLOAT_TYPE		: 
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_FLOAT_TYPE\n");
        				 F_D_FLOAT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_DOUBLE_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_DOUBLE_TYPE\n");
        				 F_D_DOUBLE_TYPE(interp,"",OUTPUT);
        				 break;
        case D_STRING_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_STRING_TYPE\n");
        				 F_D_STRING_TYPE(interp,"",OUTPUT);
        				 break;
        case D_INT_FLOAT_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_INT_FLOAT_TYPE\n");
        				 F_D_INT_FLOAT_TYPE(interp,"",OUTPUT);
        				 break;
        case D_FLOAT_READPOINT		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_FLOAT_READPOINT\n");
        				 F_D_FLOAT_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_STATE_FLOAT_READPOINT	:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_STATE_FLOAT_READPOINT\n");
        				 F_D_STATE_FLOAT_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_LONG_READPOINT		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_LONG_READPOINT\n");
        				 F_D_LONG_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_DOUBLE_READPOINT		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_DOUBLE_READPOINT\n");
        				 F_D_DOUBLE_READPOINT(interp,"",OUTPUT);
        				 break;
        case D_VAR_CHARARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_CHARARR\n");
        				 F_D_VAR_CHARARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_STRINGARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_STRINGARR\n");
        				 F_D_VAR_STRINGARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_USHORTARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_USHORTARR\n");
        				 F_D_VAR_USHORTARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_SHORTARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_SHORTARR	\n");
        				 F_D_VAR_SHORTARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_ULONGARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_ULONGARR\n");
        				 F_D_VAR_ULONGARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_LONGARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_LONGARR\n");
        				 F_D_VAR_LONGARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_FLOATARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_FLOATARR\n");
        				 F_D_VAR_FLOATARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_DOUBLEARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_DOUBLEARR\n");
        				 F_D_VAR_DOUBLEARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_FRPARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_FRPARR\n");
        				 F_D_VAR_FRPARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_SFRPARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_SFRPARR\n");
        				 F_D_VAR_SFRPARR(interp,"",OUTPUT);
        				 break;
        case D_VAR_LRPARR		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_VAR_LRPARR\n");
        				 F_D_VAR_LRPARR(interp,"",OUTPUT);
        				 break;
        case D_OPAQUE_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_OPAQUE_TYPE\n");
        				 F_D_OPAQUE_TYPE(interp,"",OUTPUT);
        				 break;
#if 0
        case D_MULMOVE_TYPE		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_MULMOVE_TYPE	\n");
        				 F_D_MULMOVE_TYPE(interp,"",OUTPUT);
        				 break;
        case D_MOTOR_LONG		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_MOTOR_LONG\n");
        				 F_D_MOTOR_LONG(interp,"",OUTPUT);
        				 break;
        case D_MOTOR_FLOAT		:
                 if(TCL_TEST) printf("dev_io: ArgoutRequired= D_MOTOR_FLOAT\n");
...
if (GetIoMode()==SYNCHRONOUS_MODE)
   {
    if (TCL_TEST) {printf("Call dev_putget...%d\n",Cmd);}
    DeviceStatus = dev_putget(CurrentDevice->ds,Cmd,\
                              InputArg,ArginRequired,\
                              OutputArg,ArgoutRequired,\
                              &DeviceError);
    /* GP TESTS */
    if (TCL_TEST) {printf("dev_putget returns: error=%d :%s\n",DeviceError,StringTemp);
                   fflush(stdout);
                  }
...
else
   {
    ...
    DeviceStatus = dev_put_asyn(CurrentDevice->ds,Cmd,\
                                InputArg,ArginRequired,\
                                &DeviceError);
   }
...
if (DeviceStatus != DS_OK) /* Error during dev_putget */
        {
         free (StringTemp); 
	 free (StringAux);
         if (OutputArg != NULL) {free (OutputArg); OutputArg=NULL;} 
         return TCL_OK;
        }
...
if (TCL_TEST) {printf("Converting the output argument in the tcl format...\n");}
...
switch (ArgoutRequired)
       {
        ...
        case D_BOOLEAN_TYPE      	:
        				  ...
        				  sprintf(TclAux,"%d",*((char*)OutputArg));        
                  Tcl_AppendResult(interp,TclAux,NULL);
                  break;
        case D_USHORT_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
        				 sprintf(TclAux,"%d",*((unsigned short*)OutputArg));
        				 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_SHORT_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
        				 sprintf(TclAux,"%d",*((short*)OutputArg));
        				 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_ULONG_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
        				 sprintf(TclAux,"%d",*((unsigned long*)OutputArg));
        				 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_LONG_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
        				 sprintf(TclAux,"%d",*((long*)OutputArg));
        				 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_FLOAT_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
        ...
        case D_DOUBLE_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
 /*************************************************************************/                
        				 sprintf(TclAux,"%17.8e",*((double*)OutputArg));
/*        				 printf("TclAux=%17.8e\n",*((double*)OutputArg));       */
/*        				 fflush stdout;                                         */
/**************************************************************************/                    
        				 Tcl_AppendResult(interp,TclAux,NULL);
        ...
        case D_STRING_TYPE		:
                 ...
                 if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
                 {
                    ...
                    sprintf (StringTemp,"%s [%d]",StringAux,DeviceError);
                    Tcl_SetVar (interp,Error,StringTemp,0);
                    free (StringTemp); free (StringAux);
                    return TCL_OK;
                 }
        				 break;
        case D_INT_FLOAT_TYPE		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
                 sprintf(TclAux,"%d",(((DevIntFloat*)OutputArg)->state));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%f",(((DevIntFloat*)OutputArg)->value));
                 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_FLOAT_READPOINT		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
                 sprintf(TclAux,"%f",(((DevFloatReadPoint*)OutputArg)->set));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%f",(((DevFloatReadPoint*)OutputArg)->read));
                 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_STATE_FLOAT_READPOINT	:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
                 sprintf(TclAux,"%d",(((DevStateFloatReadPoint*)OutputArg)->state));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%g",(((DevStateFloatReadPoint*)OutputArg)->set));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%g",(((DevStateFloatReadPoint*)OutputArg)->read));
                 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_LONG_READPOINT		:
                 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
                 sprintf(TclAux,"%d",(((DevLongReadPoint*)OutputArg)->set));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%d",(((DevLongReadPoint*)OutputArg)->read));
                 Tcl_AppendResult(interp,TclAux,NULL); 
        				 break;
        case D_DOUBLE_READPOINT		:
              	 TclAux = (char *) malloc (TCLAUX_LENGTH*sizeof(char));
              	 sprintf(TclAux,"%g",(((DevDoubleReadPoint*)OutputArg)->set));
              	 Tcl_AppendResult(interp,TclAux," ",NULL);
              	 sprintf(TclAux,"%f",(((DevDoubleReadPoint*)OutputArg)->read));
              	 Tcl_AppendResult(interp,TclAux,NULL); 
        				 break;
        case D_VAR_CHARARR		:
            	 ...
            	 for (i=0;i<j;i++)
                	 {
                  	...
                  	sprintf(TclAux,"%d",*((unsigned short*)TclTemp));
             ...
             for (i=0;i<j;i++)
                 {
                  ...
                  sprintf(TclAux,"%d",*((short*)TclTemp));
                  Tcl_AppendElement(interp,TclAux);
                 }
             DeviceStatus = dev_xdrfree(ArgoutRequired,(void*)OutputArg,&DeviceError);
             if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
               ...
               for (i=0;i<j;i++)
                   {
                    ...
                    sprintf(TclAux,"%d",*((unsigned long*)TclTemp));
                    Tcl_AppendElement(interp,TclAux);
                   }
               DeviceStatus = dev_xdrfree(ArgoutRequired,(void*)OutputArg,&DeviceError);
               if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
               ...
               for (i=0;i<j;i++)
                   {
                    ...
                    sprintf(TclAux,"%17.8e",*((double*)TclTemp));
                    Tcl_AppendElement(interp,TclAux);
                   }
               DeviceStatus = dev_xdrfree(ArgoutRequired,(void*)OutputArg,&DeviceError);
               if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
               ...
               for (i=0;i<j;i++)
                   {
                    ...
                    sprintf(TclAux,"%g",((DevFloatReadPoint*)TclTemp)->set);
                    Tcl_AppendElement(interp,TclAux);
                    sprintf(TclAux,"%g",((DevFloatReadPoint*)TclTemp)->read);
                    Tcl_AppendElement(interp,TclAux);
                   }
               DeviceStatus = dev_xdrfree(ArgoutRequired,(void*)OutputArg,&DeviceError);
               if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
            	 ...
            	 for (i=0;i<j;i++)
                	 {
                  	...
                  	sprintf(TclAux,"%d",((DevStateFloatReadPoint*)TclTemp)->state);
                  	Tcl_AppendElement(interp,TclAux);
                  	sprintf(TclAux,"%g",((DevStateFloatReadPoint*)TclTemp)->set);
                  	Tcl_AppendElement(interp,TclAux);
                  	sprintf(TclAux,"%g",((DevStateFloatReadPoint*)TclTemp)->read);
                  	Tcl_AppendElement(interp,TclAux);
                	 }
            	 DeviceStatus = dev_xdrfree(ArgoutRequired,(void*)OutputArg,&DeviceError);
            	 if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
               ...
               for (i=0;i<j;i++)
                   {
                    ...
                    sprintf(TclAux,"%d",((DevLongReadPoint*)TclTemp)->set);
                    Tcl_AppendElement(interp,TclAux);
                    sprintf(TclAux,"%d",((DevLongReadPoint*)TclTemp)->read);         
                    Tcl_AppendElement(interp,TclAux);
                   }
               DeviceStatus = dev_xdrfree(ArgoutRequired,(void*)OutputArg,&DeviceError);
               if (DeviceStatus != DS_OK) /* Error in the dev_xdrfree */
        ...
        case D_MULMOVE_TYPE		:
             ...
             for (i=0;i<8;i++)
                 {
                  sprintf(TclAux,"%d",&( ((DevMulMove*)OutputArg)->action[i]));
                  Tcl_AppendResult(interp,TclAux," ",NULL);
                 }
             for (i=0;i<8;i++)
                 {
                  sprintf(TclAux,"%d",&( ((DevMulMove*)OutputArg)->delay[i]));
                  Tcl_AppendResult(interp,TclAux," ",NULL);
                 }
             for (i=0;i<8;i++)
                 {
                  sprintf(TclAux,"%f",&( ((DevMulMove*)OutputArg)->position[i]));
                  Tcl_AppendResult(interp,TclAux," ",NULL);
                 }
        				 break;
        case D_MOTOR_LONG		: 
                 TclAux = (char*) malloc (TCLAUX_LENGTH*sizeof(char));
                 sprintf(TclAux,"%d",&( ((DevMotorLong*)OutputArg)->axisnum));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%d",&( ((DevMotorLong*)OutputArg)->value));
                 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
        case D_MOTOR_FLOAT		:
                 TclAux = (char*) malloc (TCLAUX_LENGTH*sizeof(char));
                 sprintf(TclAux,"%d",&( ((DevMotorFloat*)OutputArg)->axisnum));
                 Tcl_AppendResult(interp,TclAux," ",NULL);
                 sprintf(TclAux,"%f",&( ((DevMotorFloat*)OutputArg)->value));
                 Tcl_AppendResult(interp,TclAux,NULL);
        				 break;
#endif
        default				:
...
if (TCL_TEST) {printf("free the memory....\n");}

if (TCL_TEST) {printf("...................TclAux %X\n",TclAux);}       
if (TclAux != NULL) free (TclAux);
if (TCL_TEST) {printf("...................SequencePointer %X\n",SequencePointer);}
if (SequencePointer != NULL) free (SequencePointer);
if (TCL_TEST) {printf("...................StringTemp %X\n",StringTemp);}
if (StringTemp != NULL) free (StringTemp);
if (TCL_TEST) {printf("...................StringAux %X\n",StringAux);}
if (StringAux != NULL) free (StringAux);
if (TCL_TEST) {printf("...................StringType %X\n",StringType);}
if (StringType != NULL) free (StringType); 

if (ArginRequired == D_VAR_STRINGARR) {
  if (TCL_TEST) {printf("...................ArginRequired == D_VAR_STRINGARR\n");}
...
if (TCL_TEST) {printf("...................InputArg %X\n",InputArg);} 
if (InputArg != NULL) {free (InputArg); InputArg=NULL;}
}

if (ArgoutRequired != D_STRING_TYPE) {
if (TCL_TEST) {printf("...................OutputArg %X\n",OutputArg);} 
if (OutputArg != NULL) {free (OutputArg); OutputArg=NULL;}
}


if (TCL_TEST) {printf("End of free\n");}

if (TCL_TEST) {printf("leaving dev_io function...\n");}

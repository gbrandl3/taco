...
#--------------------------------------------------------------
	if Tab_dev.has_key(mdevname):
		...
		if (mode != "tcp") and (mode != "udp"):
			...
			return 0
		try:
			ret = Taco.esrf_tcpudp(loc_c_pt,mode)
			return 1
		except Taco.error:
			raise Dev_Exception, sys.exc_value
			return 0
...
#--------------------------------------------------------------
	if Tab_dev.has_key(mdevname):
		...
		if (mtime == ()):
			...
			try:
				ret = Taco.esrf_timeout(loc_c_pt)
				return ret
			except Taco.error:
				raise Dev_Exception, sys.exc_value
				return 0
			...
			if (type(itime) == type(0)) or (type(itime) == type(2.4)):
				...
				try:
					ret = Taco.esrf_timeout(loc_c_pt,itime)
					return ret
				except:
               				raise Dev_Exception, sys.exc_value
	       				return 0	    
...
#--------------------------------------------------------------
	...
	try:
		ret = Taco.esrf_putresource(mdevname,resname,value)
		return 1
	except Taco.error:
		raise Dev_Exception, sys.exc_value
		return 0
...
#--------------------------------------------------------------
	try:
		ret = Taco.esrf_delresource(mdevname,resname)
		return 1
	except Taco.error:
		raise Dev_Exception, sys.exc_value
		return 0
...
#  first check that command exists:
		if Tab_dev[mdevname]['cmd'].has_key(mdevcommand):
			...
			ret = None
			try:
				ret = apply(Taco.esrf_io,parin2,kw)
			except Taco.error:
				raise Dev_Exception, sys.exc_value
			return ret
		else:

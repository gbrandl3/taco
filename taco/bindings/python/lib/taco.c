 ...
 */
static PyObject *esrf_debug(PyObject *self, PyObject *args, PyObject *kwarg)
{
 ...
 *		NULL:			error
 */
static PyObject* esrf_query(PyObject *self, PyObject *args, PyObject *kwarg)
{
	int			i;
	...
	PyObject		*py_ds_pt,
				...
				*item;
	static devserver	*ds;
	static char		*kwlist[] = {"cds", NULL};

	if (flag != 0)
		printf("-- esrf_query: enter\n");
	/* get pointer on device server */
	if (!PyArg_ParseTupleAndKeywords(args, kwarg, "O", kwlist, &py_ds_pt))
...
static PyObject *esrf_getresource(PyObject *self, PyObject *args)
{
	...
	else
	{
		...
		{
			PyObject *tmp = Py_BuildValue("s", res_array.sequence[0]);
...
static PyObject* esrf_putresource(PyObject *self, PyObject *args)
{
	...
	if (db_putresource(devname, &pt_res, num_res, &error) != 0)
	{
...
static PyObject* esrf_tcpudp(PyObject *self, PyObject *args)
{
	...
	found = 0;
	if (mymode && !strcmp(mymode, "tcp"))
	{
		if (flag != 0)
			printf("-- esrf_tcpudp: setting mode tcp \n");
		protocol = D_TCP;
	}
	else if (mymode && !strcmp(mymode, "udp"))
	{
		if (flag != 0)
...
static PyObject *esrf_timeout(PyObject *self, PyObject *args)
{
	...
	else
	{
		...
		else
		{
			...
			return Py_None; /* Py_BuildValue("f", mytime); */
		}
	}
}
...
static PyObject *esrf_getdevlist(PyObject *self, PyObject *args)
{
	...
	{
		printf("usage : esrf_getdevlist(server_name)\n");
		onError("usage: esrf_getdevlist(server_name)")
	}
	...
	if (db_getdevlist(server_name, &tab, &dev_num, &error)!= 0)
	{
...
returns:
		...
		NULL:			error
***************************************************/
static PyObject *esrf_getdevexp(PyObject *self, PyObject *args)
{
	int		i;
	PyObject	*mylist,
			*item;
	...
	if (!PyArg_ParseTuple(args, "|s", &filter))
		onError("usage: esrf_getdevexp(filter='*/*/*')")
...
static PyObject* esrf_io(PyObject *self, PyObject *args, PyObject *kwarg)
{
	...
	static char *kwlist[] = {"cds", "cmds", "cmd", "typein", "typeout", "dc",
				 "parin", "out", "outtype", NULL};
	...
	{
		printf("auxiliary arguments should be in a tuple.\n");
		onError("auxiliary arguments should be in a tuple.")
	}
	...
	else
	{
		...
		/* then copy */
		memcpy((void *)array_2py, (void *)(((DevVarCharArray*)(ds_argout))->sequence),

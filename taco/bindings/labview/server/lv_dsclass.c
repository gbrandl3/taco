...
long lv_ds__cmd_get(void **lv_argin)
{
	...
	{
		printf("lv_dsclass::lv_ds__cmd_get: copy to nonstatic var");
		*lv_argin = lv_ds__argin;
		printf("lv_dsclass::lv_ds__cmd_get: copy to nonstatic var DONE");
	}	
...
long lv_ds__cmd_put(void *lv_argout)
{
   ...
   int i;
	...
	printf("entering lv_ds__cmd_put:\n\n");
	
	/*for (i = 0; i < ((DevVarStringArray *)lv_argout)->length; ++i) {
		printf("string out a : %s\n", ((DevVarStringArray *)lv_argout)->sequence[i]);
	}*/
	printf("cmd_in: %d\n\n", cmd_in);
	printf("LVGetViInfo: %d\n", LVGetViInfo);
	printf("LVGetControlList: %d\n", LVGetControlList);
	printf("LVGetControlInfo: %d\n", LVGetControlInfo);
	printf("LVGetIndicatorList: %d\n", LVGetIndicatorList);
	printf("LVGetIndicatorInfo: %d\n", LVGetIndicatorInfo);
	if (lv_ds__argout != NULL) {
		printf("not null\n");
	}else {
		printf("null\n");
	}
	...
	{
		printf("now testing first arg:\n\n");
		printf("String in [0] = %s\n",((DevVarStringArray *)lv_argout)->sequence[0]);
...
long 		*error;
{
   ...
   printf("LabViewGeneric dev_getviinfo()\n");
	...
	printf("dev_getviinfo: assigning pointer\n");
   lv_ds__argout = (void*)argout;
	printf("dev_getviinfo: done\n");
	return(DS_OK);
}
...
long 		*error;
{
   ...
   //printf("trying to copy the string %s",(char*)(*argin));
   //strcpy(lv_ds__ctrl_name_in, *argin);
   //printf("strcpy succsessful");

 ...
 *
 * File:        db_devdomain.cpp
 *
 * Description: Code for db_devdomain command                                  
 *    		Command rule : To get a list of domains of the device from the 
 *		static database.          
...
//
	long res_nb;
	char **tmp; 
	if (db_getdevdomainlist(&res_nb, &tmp, &error) == -1)
	{

...
dnl   ---------- >8 ----------
dnl
dnl @version $Id: am_rpm_init.m4,v 1.1 2003-04-25 13:30:26 jkrueger1 Exp $
...
AC_ARG_WITH(rpm-prog, AC_HELP_STRING([--with-rpm-prog=PROG], [Which rpm to use (optional)]),
            rpm_prog="$withval", rpm_prog="")

AC_ARG_ENABLE(rpm-rules, AC_HELP_STRING([--enable-rpm-rules], [Try to create rpm make rules (defaults to yes for Linux)]),
                enable_rpm_rules="$withval",enable_rpm_rules=yes)

AC_ARG_WITH(rpm-extra-args, AC_HELP_STRING([--with-rpm-extra-args=ARGS], [Run rpm with extra arguments (defaults to none)]),
                rpm_extra_args="$withval", rpm_extra_args="")

dnl AC_ARG_ENABLE(rpm-topdir, AC_HELP_STRING([--enable-rpm], [Try to create rpm make rules (defaults to yes for Linux)]),

...
/****************************************************************************
 ...
 $Date: 2003-04-25 11:21:40 $			$State: Exp $
...
static long _asc_load_library()
{
   ...
   if (_asc_TC_shl == NULL)
   {
      printf("_asc_load_library : Cannot load the type conversion shared library.");
      return(-1);
   }
#else
   ...
   if (_asc_TC_shl == NULL)
   {
      printf("_asc_load_library : Cannot load the type conversion shared library(lib_path=%s)",lib_path);
      return(-1);
   }
#endif /* __hpux */
...
{
   int    ind_dev, dev_found;

   ind_dev = 0;
   dev_found = ASC_FALSE;

   while ( (dev_found == ASC_FALSE) &&
	   (ind_dev < (ASC_MAX_DEV * _asc_nb_block_dev_array)) )
   {
   ...
   else
   {
	      ...
	      (ind_dev < (ASC_MAX_DEV * _asc_nb_block_dev_array)) )
      {
	    ...
	    if ( strcasecmp(asc_dev_array_ptr[ind_dev].dev_name, ASC_EMPTY_DEV)==0)
	       dev_found = ASC_TRUE;
            else
	       ind_dev++;
      }
...
static long  _asc_dev_array_realloc()
{
   ...
   int                 new_size, index_dev;

   new_size = ((_asc_nb_block_dev_array + 1) * ASC_MAX_DEV)
	      * sizeof(_asc_dev_info_type);
   dev_array_safe = asc_dev_array_ptr;

   asc_dev_array_ptr = (_asc_dev_info_type *)
		       realloc(asc_dev_array_ptr,new_size);
...
static long _asc_lookup_cmd(int index_dev, const char *cmd_name,
		     long *cmd_code, long *in_type, long *out_type)
{
   int              ind_cmd, cmd_found;
...
static long _asc_dev_connect(char *dev_name, const char *cmd_name,
				       devserver *ds_ptr, long  *cmd_code,
				       long  *in_type, long *out_type,
				       char  **str_err )

{
   int                 index_dev, ind_cmd;
   int                 cmd_found;
   long                status, error;
   devserver           ds;
   ...
   {
      asc_dev_array_ptr = (_asc_dev_info_type *)
			  malloc (sizeof(_asc_dev_info_type) * ASC_MAX_DEV);
   ...
   if (index_dev >= (ASC_MAX_DEV * _asc_nb_block_dev_array))
   {
      ...
      if (status == -1)
      {
	 *str_err = (char *) malloc(51);
	 strcpy(*str_err, "Too many devices already imported. Cannot realloc");
	 strcat(*str_err, " device array.");
	 return(-1);
      }
      else
         index_dev = _asc_lookup_dev(dev_name);
   ...
   { /*** device already in the device array ***/
      ...
      *ds_ptr = asc_dev_array_ptr[index_dev].ds;
      ...
      {
	 *str_err = (char *) malloc(51);
	 strcpy(*str_err, "Cannot find the command for this device.");
	 return(-1);
      }
      else
         return(0);
   }
   else /*** device is not yet in the dev array so should do dev_import ***/
   {
   ...
   } /*** else device is not yet in the dev array ***/
...
static long _asc_convert_outtype ( const char *type_name, 
                                   const void *data_to_convert,
                                         char *res_function,
                                         char *err_str )
{
   char  func_name[201];
   ...
   strcpy(func_name, "tc_");
   strcat(func_name, type_name);
   strcat(func_name, "_to_str");
...
static long _asc_convert_intype ( const char *type_name, 
                                  const char *str_in,
                                        void *res_convert,
                                        char *err_str )
{
...
***************************************************************************/
        ...
        status = _asc_convert_intype (type_to_convert, in_arg,
                                                       in_data, conv_err_str);
...
***************************************************************************/
	...
	if (status == -1)
	{
	   ...
	   return(-1);
        ...
        switch (in_type)
        {
               case (D_STRING_TYPE):   if (d_in_string != NULL)
                                       {
                                          free(d_in_string);
                                          d_in_string = NULL;
                                       }
                                       break;
               case (D_VAR_CHARARR):   if (d_in_char_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_STRINGARR): if (d_in_string_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_USHORTARR): if (d_in_ushort_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_SHORTARR):  if (d_in_short_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_ULONGARR):  if (d_in_ulong_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_LONGARR):   if (d_in_long_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_FLOATARR):  if (d_in_float_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_DOUBLEARR): if (d_in_dbl_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
/***************************************************************************
               case (D_VAR_FRPARR):    if (d_in_flrp_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_SFRPARR):   if (d_in_stflrp_arr.sequence != NULL)
                                       {
                                       ...
                                       break;
               case (D_VAR_LRPARR):    if (d_in_lnrp_arr.sequence != NULL)
                                       {

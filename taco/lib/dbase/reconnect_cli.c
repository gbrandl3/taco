...
#if 0
/*
 *		all seems to be ok and I can return
 */
/*
 * Redo the call
 */
...
/* Pass data to caller and leave function */
	...
	{
/*
 * Let us say that the database server is dead. Ask for a new connection to the
 * new database server. If the nethost index is 0 (default nethost), don't use 
 * the multi_nethost array. Set the API database_server and configuration flags 
 * for the right network to false in order to force the API to re-ask the manager 
 * for the new db server RPC parameters. 
 */
#ifdef ALONE
		old_client = *client;
		...
		else
		{
			...
			cl_tcp = clnttcp_create(&serv_adr,db_info.conf->prog_number, DB_VERS_3,&tcp_so,0,0);
 ...
 */
	switch (call_type)
	{
		...
		case DB_INITCACHE : 
			recev_ptr.lg_ptr = db_initcache_1((DevString *)p_data, *client,error);
			recev_gen = (void *)recev_ptr.lg_ptr;
			break;
